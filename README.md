...
> **CI status:** the Unit Tests workflow installs `pocl` (a CPU-based OpenCL implementation) as the ICD before running the suite on the Ubuntu runner, so `LibraryNativeOpenCLRudesheim` can resolve `libOpenCL.so` and `clinfo` correctly reports one platform/device. The workflow currently still fails, though: calling `clGetDeviceIDs` through this package's threaded FFI (`TFWorker`, which runs the native call on a dedicated worker thread) segfaults inside `pocl`'s own code on the GitHub Actions runner, even with verified-correct arguments (confirmed by temporarily logging the marshaled platform pointer, device-type mask, and buffer pointers immediately before the call). This looks like a `pocl`/threaded-FFI-worker incompatibility specific to that containerized environment rather than a code defect here — while investigating, we also found and fixed a real, independent bug: `clGetDeviceIDs`'s `device_type` parameter was declared as a 32-bit FFI type instead of the 64-bit `cl_bitfield` the OpenCL spec requires. Locally the suite passes against a real GPU/vendor OpenCL runtime (182 Tests, 0 Failures, 0 Errors).
